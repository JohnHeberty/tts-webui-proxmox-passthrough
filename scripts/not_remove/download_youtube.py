...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("train/logs/download_youtube.log"), logging.StreamHandler()],
)
            ...
            with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                ...
                if not os.path.exists(output_path):
                    raise FileNotFoundError(
                        f"Arquivo {output_filename} não foi criado após o download"
                    )
...
def main():
    ...
    try:
        ...
        logger.error(
            "❌ ffmpeg não encontrado. Instale com: apt install ffmpeg (Linux) ou brew install ffmpeg (Mac)"
        )
        sys.exit(1)
    ...
    for i, video_info in enumerate(videos, 1):
        ...
        output_filename = (
            output_filename + ".wav" if ".wav" not in output_filename else output_filename
        )
