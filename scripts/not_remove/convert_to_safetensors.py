...
def load_checkpoint(checkpoint_path: Path) -> dict[str, Any]:
    ...
    try:
        checkpoint = torch.load(
            checkpoint_path, map_location="cpu", weights_only=False  # Necessário para F5-TTS
        )
...
def create_metadata(checkpoint: dict[str, Any]) -> dict[str, str]:
    ...
    if "ema_model_state_dict" in checkpoint:
        ema_dict = checkpoint["ema_model_state_dict"]
        total_params = sum(p.numel() for p in ema_dict.values() if isinstance(p, torch.Tensor))
...
def convert_checkpoint_to_safetensors(
    checkpoint_path: Path, output_path: Path | None = None
) -> Path:
    """
    ...
    try:
        save_file(state_dicts, output_path, metadata=metadata)
    ...
    try:
        ...
        logger.info("📋 Amostra de tensores:")
...
Exemplos:
    ...
    try:
        # Executar conversão
        output_file = convert_checkpoint_to_safetensors(checkpoint_path, output_path)

        logger.info("\n🎉 Arquivo SafeTensors criado:")
        logger.info(f"   {output_file}")

        logger.info("\n💡 Para usar este modelo:")
        logger.info("   from safetensors.torch import load_file")
