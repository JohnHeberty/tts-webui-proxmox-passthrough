...
class HealthCheck:
        ...
        try:
            ...
            if torch.cuda.is_available():
                ...
                if gpu_mem_gb >= 16:
                    ...
                    self.check_warn(
                        f"GPU memory: {gpu_mem_gb:.1f} GB (may be tight, 16GB+ recommended)"
                    )
                else:
                    self.check_fail(
                        f"GPU memory: {gpu_mem_gb:.1f} GB (insufficient, need 8GB+ minimum)"
                    )
    ...
    def check_disk_space(self) -> bool:
        ...
        try:
            ...
            if free_gb >= 50:
                ...
                self.check_fail(
                    f"Disk space: {free_gb:.1f} GB free (insufficient, need 20GB+ minimum)"
                )
                return False
    ...
    def check_dataset(self, dataset_path: str = "train/data/f5_dataset") -> bool:
        ...
        if wavs_dir.exists():
            ...
            self.check_warn("Audio directory not found: wavs/")
...
def main():
    ...
    parser.add_argument(
        "--dataset-path", default="train/data/f5_dataset", help="Path to dataset directory"
    )
    parser.add_argument(
        "--vocab-path", default="train/config/vocab.txt", help="Path to vocabulary file"
    )
    parser.add_argument(
        "--config-path", default="train/config/base_config.yaml", help="Path to config file"
    )
