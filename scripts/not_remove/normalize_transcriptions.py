...
def main():
    ...
    for i, item in enumerate(transcriptions, 1):
        ...
        if i % 100 == 0:
            logger.info(
                f"   Progresso: {i}/{len(transcriptions)} ({i/len(transcriptions)*100:.1f}%)"
            )
    ...
    logger.info(
        f"   Normalizadas: {stats['normalized']} ({stats['normalized']/stats['total']*100:.1f}%)"
    )
    logger.info(
        f"   Sem alteração: {stats['unchanged']} ({stats['unchanged']/stats['total']*100:.1f}%)"
    )
