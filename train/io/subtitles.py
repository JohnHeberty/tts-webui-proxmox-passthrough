...
if __name__ == "__main__":
    ...
    extract_parser.add_argument(
        "--no-auto", action="store_true", help="Disable auto-generated subtitles"
    )
    ...
    parse_parser.add_argument(
        "--merge", type=float, default=0.0, help="Merge lines with gap <= N seconds"
    )
    ...
    if args.command == "extract":
        ...
        for sub in subtitles[:5]:
            print(
                f"  {sub.index}. [{sub.start_time:.1f}s - {sub.end_time:.1f}s] ({sub.duration:.1f}s)"
            )
            print(f"     {sub.text}")
