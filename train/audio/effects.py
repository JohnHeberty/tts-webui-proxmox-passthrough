...
def apply_high_pass_filter(
    ...
    try:
        ...
        if normalized_cutoff <= 0 or normalized_cutoff >= 1:
            ...
            warnings.warn(
                f"Invalid cutoff frequency {cutoff_hz}Hz for sample rate {sample_rate}Hz",
                UserWarning,
            )
            return audio
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--fade-in", type=float, default=5.0, help="Fade-in duration in ms (default: 5.0)"
    )
    parser.add_argument(
        "--fade-out", type=float, default=5.0, help="Fade-out duration in ms (default: 5.0)"
    )
    parser.add_argument("--high-pass", type=float, help="High-pass cutoff frequency in Hz")
    parser.add_argument("--low-pass", type=float, help="Low-pass cutoff frequency in Hz")
