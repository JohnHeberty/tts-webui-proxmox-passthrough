...
app = typer.Typer(
    name="f5tts-infer",
    help="F5-TTS Inference CLI - Quick testing tool",
...
def infer(
    ...
    if ref_text:
        param_table.add_row("Reference Text", ref_text[:60] + "..." if len(ref_text) > 60 else ref_text)
...
def info(
    ...
    try:
        ...
        info_table.add_row("Format", "SafeTensors" if str(checkpoint).endswith('.safetensors') else "PyTorch")
