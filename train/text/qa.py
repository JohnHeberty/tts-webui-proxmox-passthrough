...
if __name__ == "__main__":
    ...
    check_parser.add_argument(
        "--normalize", action="store_true", help="Normalize text before checking"
    )
    ...
    batch_parser.add_argument(
        "--normalize", action="store_true", help="Normalize texts before checking"
    )
    ...
    if args.command == "check":
        valid, processed, issues = validate_text_for_training(
            args.text, args.vocab, normalize=args.normalize, verbose=True
        )
        ...
        for i, text in enumerate(texts, 1):
            valid, processed, issues = validate_text_for_training(
                text, args.vocab, normalize=args.normalize, verbose=False
            )
