...
def validate_checkpoint(
    checkpoint_path: str, min_size_gb: float = 1.0, check_keys: bool = True, verbose: bool = True
) -> CheckpointInfo:
    """
    ...
    try:
        ...
        error_msg = f"Failed to load: {e!s}"
        info = CheckpointInfo(
            path=checkpoint_path,
            exists=True,
            size_bytes=size_bytes,
...
def mark_checkpoint_corrupted(checkpoint_path: str, verbose: bool = True) -> bool:
    ...
    try:
        ...
        if verbose:
            logger.warning(
                f"🗑️  Marked as corrupted: {checkpoint_path.name} → {corrupted_path.name}"
            )
        return True
    except Exception as e:
        if verbose:
...
def resolve_checkpoint_path(
    config, checkpoint_name: str | None = None, auto_download: bool = True, verbose: bool = True
) -> str | None:
    """
...
def download_pretrained_model(config, verbose: bool = True) -> str | None:
    """
...
def save_checkpoint_metadata(
    checkpoint_path: str, metadata: dict[str, Any], verbose: bool = True
) -> bool:
    """
...
def load_checkpoint_metadata(checkpoint_path: str) -> dict[str, Any] | None:
    """
...
if __name__ == "__main__":
    ...
    validate_parser.add_argument("--min-size", type=float, default=1.0, help="Minimum size in GB")
    ...
    resolve_parser.add_argument("--no-download", action="store_true", help="Disable auto-download")
    ...
    if args.command == "validate":
        info = validate_checkpoint(args.checkpoint_path, min_size_gb=args.min_size, verbose=True)
        print(f"\n{info}")
        ...
        checkpoint_path = resolve_checkpoint_path(
            config, checkpoint_name=args.name, auto_download=not args.no_download, verbose=True
        )
