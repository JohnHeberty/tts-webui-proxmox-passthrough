...
"""
    ...
    # 1. Download
    run_command("python -m train.scripts.download_youtube", "1/6 - Download de áudio do YouTube")

    # 2. Segmentação
    run_command("python -m train.scripts.prepare_segments", "2/6 - Segmentação de áudio")

    # 3. Transcrição
    run_command("python -m train.scripts.transcribe_or_subtitles", "3/6 - Transcrição de áudio")

    # 4. Metadata
    run_command("python -m train.scripts.build_metadata_csv", "4/6 - Construção do metadata.csv")

    # 5. Dataset
    run_command("python -m train.scripts.prepare_f5_dataset", "5/6 - Preparação do dataset F5-TTS")
