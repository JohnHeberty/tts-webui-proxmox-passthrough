...
def apply_cli_overrides(
    config_dict: dict[str, Any], cli_args: dict[str, Any] | None = None
) -> dict[str, Any]:
    """
...
def print_config_summary(config: F5TTSConfig):
    ...
    print(
        f"  Batch Size: {config.training.batch_size_per_gpu} (grad_accum={config.training.grad_accumulation_steps})"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--config", type=str, default="train/config/base_config.yaml", help="Path to config file"
    )
    parser.add_argument("--save", type=str, default=None, help="Save final merged config to file")
    parser.add_argument("--no-validate", action="store_true", help="Skip Pydantic validation")

    args = parser.parse_args()

    try:
        config = load_config(config_path=args.config, validate=not args.no_validate)
