...
class PathsConfig(BaseModel):
    ...
    vocab_file: str = Field("train/config/vocab.txt", description="Vocabulary file (SOURCE OF TRUTH)")
    
    pretrained_dir: str = Field("train/pretrained", description="Pretrained models directory")
    output_dir: str = Field("train/output/ptbr_finetuned2", description="Output checkpoints directory")
    pretrained_model_path: str = Field("train/pretrained/F5-TTS-pt-br/pt-br/model_200000_fixed.pt", description="Pretrained model path")
...
class ModelConfig(BaseModel):
    ...
    custom_checkpoint: Optional[str] = Field(None, description="Custom checkpoint path (overrides pretrained)")
...
class VocoderConfig(BaseModel):
    ...
    local_path: Optional[str] = Field(None, description="Local vocoder path")
...
class TrainingConfig(BaseModel):
    ...
    batch_size_type: str = Field("sample", description="Batch size type", pattern="^(sample|frame)$")
    ...
    early_stop_min_delta: float = Field(0.001, ge=0.0, description="Min improvement for early stopping")
...
class MixedPrecisionConfig(BaseModel):
    ...
    dtype: Optional[str] = Field("fp16", description="Data type", pattern="^(fp16|bf16|null)$")
...
class WandBConfig(BaseModel):
    ...
    entity: Optional[str] = Field(None, description="W&B entity (username/team)")
...
class LoggingConfig(BaseModel):
    ...
    logger: str = Field("tensorboard", description="Logger type", pattern="^(tensorboard|wandb|null)$")
...
class ValidationConfig(BaseModel):
    ...
    val_dataset_path: Optional[str] = Field(None, description="Validation dataset path")
...
class AdvancedConfig(BaseModel):
    ...
    f5tts_base_dir: Optional[str] = Field(None, description="F5-TTS base directory")
    f5tts_ckpts_dir: Optional[str] = Field(None, description="F5-TTS checkpoints directory")
...
class ASRConfig(BaseModel):
    ...
    model: str = Field("base", description="Whisper model", pattern="^(tiny|base|small|medium|large|large-v2)$")
    ...
    vad_filter_min_silence_duration: float = Field(0.5, ge=0.0, description="VAD min silence (seconds)")
...
class YouTubeConfig(BaseModel):
    ...
    subtitles: SubtitlesConfig = Field(default_factory=SubtitlesConfig, description="Subtitles config")
...
class QualityFiltersConfig(BaseModel):
    ...
    music_detection_threshold: float = Field(0.3, ge=0.0, le=1.0, description="Music detection threshold")
