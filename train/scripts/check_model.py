    ...
    try:
        ...
        if isinstance(checkpoint, dict):
            ...
            if 'ema_model_state_dict' in checkpoint:
                ...
                ema_params = sum(p.numel() for p in ema_dict.values() if isinstance(p, torch.Tensor))
...
def fix_model(checkpoint, output_path, add_ema=False):
    ...
    try:
        ...
        elif isinstance(checkpoint, dict) and any(k.startswith('model.') or k.startswith('transformer.') for k in checkpoint.keys()):
