...
def analyze_checkpoint(checkpoint_path, checkpoint_name="Checkpoint"):
    ...
    try:
        ...
        info = {
            ...
            "modified": datetime.fromtimestamp(checkpoint_path.stat().st_mtime).strftime(
                "%Y-%m-%d %H:%M:%S"
            ),
        }
...
def print_metrics_report():
    ...
    if log_path.exists():
        ...
        if metrics["epochs"]:
            ...
            for i, (epoch, loss, update) in enumerate(
                zip(metrics["epochs"], metrics["losses"], metrics["updates"])
            ):
                if i == 0:
                    improvement = "-"
                else:
            ...
            logger.info(
                f"📊 Redução total: {((metrics['losses'][0] - metrics['losses'][-1]) / metrics['losses'][0] * 100):.2f}%"
            )
    ...
    if tensorboard_dirs:
        ...
        logger.info("   tensorboard --logdir=runs")
    ...
    if metrics["epochs"]:
        ...
        logger.info(
            f"✓ Loss: {metrics['losses'][0]:.4f} → {metrics['losses'][-1]:.4f} ({((metrics['losses'][0] - metrics['losses'][-1]) / metrics['losses'][0] * 100):.1f}% redução)"
        )
