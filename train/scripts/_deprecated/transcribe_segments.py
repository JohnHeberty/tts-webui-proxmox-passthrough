...
def transcribe_segment_whisper(audio_path: Path, model: whisper.Whisper, config: dict) -> str:
    """
...
def transcribe_segments_batch(
    segments: list[dict], model: whisper.Whisper, config: dict, batch_size: int = 10
) -> list[dict]:
    """
    ...
    for i in range(0, total, batch_size):
        ...
        for j, segment in enumerate(batch, 1):
            ...
            if not audio_path.exists():
                logger.warning(
                    f"   ⚠️  [{segment_idx}/{total}] Arquivo não encontrado: {audio_rel_path}"
                )
                continue
            ...
            if not processed_text:
                logger.warning("   ⚠️  Texto muito curto após preprocessamento")
                continue
...
def main():
    ...
    if transcriptions_file.exists():
        logger.info("📄 Arquivo de transcrições existente encontrado")
    ...
    transcribed = transcribe_segments_batch(
        segments, model, config, batch_size=10  # Processar 10 por vez para economizar memória
    )
    ...
    if transcribed:
        ...
        logger.info(
            f"📝 Tamanho médio do texto: {sum(text_lengths) / len(text_lengths):.0f} caracteres"
        )
