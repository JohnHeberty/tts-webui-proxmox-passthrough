...
def main():
    ...
    if pinyin_dir.exists():
        ...
        print(
            f"{YELLOW}⚠️{RESET}  Symlink f5_dataset_pinyin não existe (será criado automaticamente)"
        )
    ...
    if checkpoint_path.exists():
        ...
        try:
            ...
            all_ok &= check(
                "update" in ckpt, f"Campo update presente (valor: {ckpt.get('update', 'N/A')})"
            )
    ...
    if env_file.exists():
        with open(env_file) as f:
            content = f.read()
            all_ok &= check(
                "PRETRAIN_MODEL_PATH=ckpts/f5_dataset/model_last.pt" in content,
                "PRETRAIN_MODEL_PATH configurado corretamente",
            )
            all_ok &= check("USE_FINETUNE_FLAG=true" in content, "USE_FINETUNE_FLAG=true")
