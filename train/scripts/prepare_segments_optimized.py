...
def split_segment_by_duration(
    ...
    overlap = seg_config.get("segment_overlap", 0.0)
...
def resample_segment(
    ...
    """
    Faz resample do segmento. Se scipy.signal estiver disponível, usa método
    melhor; senão, faz decimação/repetição simples.
    """
    ...
    if signal is not None:
        try:
            ...
            audio_rs = signal.resample(audio, num_samples).astype(np.float32, copy=False)
            return audio_rs, target_sr
        except Exception as e:
            logger.warning(f"   ⚠️  Falha ao fazer resample com scipy: {e}")
...
def process_audio_file(
    ...
    if use_vad:
        ...
        logger.info(f"   Segmentos com voz detectados (antes do merge): {len(all_voice_segments)}")
    ...
    # Extração de segmentos + normalização + resample
...
def main():
    ...
    if all_segments:
        durations = np.array([seg["duration"] for seg in all_segments], dtype=np.float32)
