...
class TranscriptionValidator:
    ...
    def validate(self, text: str, filename: str) -> tuple[bool, list[str]]:
        ...
        unknown_words = [
            w
            for w in words
            if len(w) > 3 and w.lower() not in self.portuguese_words and not w.isdigit()
        ]
        if len(words) > 0:
            unknown_ratio = len(unknown_words) / len(words)
            if unknown_ratio > 0.7:  # >70% palavras desconhecidas
                issues.append(
                    f"Muitas palavras desconhecidas: {unknown_ratio:.1%} ({unknown_words[:5]}...)"
                )

        is_valid = len(issues) == 0

        if issues:
            self.issues_found.append({"filename": filename, "text": text[:100], "issues": issues})
...
def main():
    ...
    for item in transcriptions:
        ...
        filename = (
            Path(audio_path).name if audio_path else f"segment_{item.get('segment_index', 0)}"
        )
        text = item["text"]
        ...
        if not is_valid:
            invalid_items.append({"item": item, "issues": issues})
    ...
    logger.info("\n" + "=" * 80)
    logger.info("🔄 Re-processamento com modelo 'medium'")
    logger.info("=" * 80)
    logger.info(f"Total de áudios a re-processar: {len(invalid_items)}")
    logger.info("Modelo: whisper-medium (mais preciso que base)")
    ...
    logger.info("\n" + "=" * 80)
    logger.info("✅ RE-PROCESSAMENTO CONCLUÍDO")
    logger.info("=" * 80)
