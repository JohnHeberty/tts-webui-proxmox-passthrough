...
def main():
    ...
    if dataset_path.exists():
        ...
        all_ok &= check_emoji(
            duration.exists(),
            f"duration.json encontrado",
            "duration.json não encontrado!"
        )
        ...
        if vocab.exists():
            ...
            all_ok &= check_emoji(
                True,
                f"vocab.txt encontrado ({vocab_size} tokens)",
                ""
            )
        else:
            all_ok &= check_emoji(
    ...
    pretrain_path = os.getenv('PRETRAIN_MODEL_PATH', 'train/pretrained/F5-TTS-pt-br/pt-br/model_200000_fixed.pt')
    ...
    if pretrain_path.exists():
        ...
        all_ok &= check_emoji(
            True,
            f"Modelo encontrado: {pretrain_path.name} ({size_mb:.1f} MB)",
            ""
        )
        ...
        all_ok &= check_emoji(
            False,
            "",
            f"Modelo não encontrado: {pretrain_path}"
        )
        print(f"     Execute: python3 -m train.scripts.check_model <caminho_modelo> --fix")
    ...
    for module, name in dependencies:
        try:
            ...
            all_ok &= check_emoji(
                False,
                "",
                f"{name} não instalado! Execute: pip install {module}"
            )
    ...
    all_ok &= check_emoji(
        runs_dir.exists(),
        f"Diretório de logs criado: {runs_dir.relative_to(project_root)}",
        ""
    )
