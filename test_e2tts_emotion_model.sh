...
AUDIO_FILE="./audio_sample_pt_br.wav"

echo "=========================================="
echo "E2-TTS EMOTION MODEL TEST"
echo "Model: SWivid/E2-TTS"
echo "=========================================="
echo ""
...
for i in {1..60}; do
  ...
  if [ "$STATUS" = "completed" ]; then
    ...
    echo "   Ref Text: $(echo "$STATUS_RESPONSE" | jq -r '.ref_text // "N/A"')"
    break
...
TEXT="Olá! Este é um teste do modelo E2-TTS com suporte emocional. Estou muito feliz de testar este novo modelo de síntese de voz com emoções naturais!"
...
for i in {1..120}; do
  ...
  if [ "$STATUS" = "completed" ]; then
    ...
    if [ -f test_e2tts_emotion_output.wav ]; then
      FILE_SIZE=$(stat -f%z test_e2tts_emotion_output.wav 2>/dev/null || stat -c%s test_e2tts_emotion_output.wav)
      ...
      echo "Engine: f5tts"
      echo "Quality: ultra_quality (NFE 64)"
      echo ""
    else
